...
Local semantic search for Zotero libraries. Embeds item content (titles, abstracts, attachment fulltext) using a local ONNX model (BGE-small-en-v1.5), stores vectors in SQLite, and answers natural-language queries via cosine-similarity ranking.

No cloud APIs, no external services — everything runs on-device.

## Features

- **Paragraph-aware chunking** — splits text on paragraph boundaries, falls back to sentence separators, then hard character limits
- **Local embeddings** — ONNX-based BGE-small-en-v1.5 (384-dim) via `fastembed`, no API keys needed
- **SQLite vector store** — WAL mode, foreign keys, BLOB-encoded embeddings
- **Incremental indexing** — skips unchanged items by `dateModified`, removes stale entries
- **Cosine-similarity search** — L2-normalized dot product, best-chunk-per-item deduplication
...
println!("Indexed {} items, {} chunks", report.items_indexed, report.chunks_written);

// 4. Search
let chunks = index.load_all_chunks().await?;
let hits = search_library(&provider, &chunks, "quantum entanglement", 10, 0.3).await?;
for hit in &hits {
    println!("[{:.3}] {} — {}", hit.similarity, hit.title.as_deref().unwrap_or("untitled"), &hit.chunk_text[..80]);
}
```
...
1. **Indexing** — fetches all items from the Zotero API, assembles text (title + abstract + first child attachment fulltext), chunks it, embeds in batches of 32, L2-normalizes, and upserts into SQLite. Stale items (deleted or modified since last index) are removed.

2. **Search** — embeds the query text, computes dot product against all stored chunks (pre-normalized), keeps the highest-scoring chunk per item above `min_similarity`, and returns results sorted by descending score.

3. **Chunking** — recursive splitting: paragraph breaks (`\n\n`) first, then sentence separators (`". "`, `"! "`, `"? "`), then hard character cuts. Ensures no chunk exceeds `MAX_CHUNK_CHARS`.

## Integration

Part of the `zotero-rs` workspace. Used by [`zotero-mcp`](../zotero-mcp/) to power the `zotero_semantic_search` MCP tool.
...
Unit tests cover chunking, embedding encode/decode, SQLite upsert/delete roundtrips, indexing with mock servers, and search ranking logic.
